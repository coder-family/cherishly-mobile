...
export default function EditFamilyGroupScreen() {
  ...
  const handleAvatarUpload = async (groupId: string) => {
    if (avatarUrl && avatarUrl !== currentGroup?.avatarUrl) {
      try {
        ...
        // Avatar upload failed, but group was updated
  ...
  const showSuccessMessage = () => {
    Alert.alert("Success!", "Family group has been updated successfully.", [
      {
        text: "OK",
        onPress: () => router.back(),
      },
  ...
  const handleUpdateError = (err: any) => {
    // Update family group error handled silently
    Alert.alert(
      "Error",
  ...
  const uploadGroupAvatar = async (groupId: string, imageUri: string) => {
    ...
    const response = await fetch(
      `${BASE_URL}/family-groups/${groupId}/avatar`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Avatar upload failed: ${errorMessage} (Status: ${response.status})`
      );
    }
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              // Delete family group error handled silently
              Alert.alert(
                "Error",
                err.message ||
                  "Failed to delete family group. Please try again."
              );
              setIsSubmitting(false);
            }
          },
              ...
              <View style={styles.infoSection}>
                ...
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoValue}>
                    {currentGroup.members?.length || 0} member
